...
export function Sidebar({ collapsed, onNavigate }: SidebarProps) {
  ...
  // Poll server list every 30s for status changes
  useEffect(() => {
  ...
  if (collapsed) {
    ...
    return (
      <aside className="flex h-full w-14 flex-col items-center border-r bg-white py-4 dark:border-gray-800 dark:bg-gray-900">
        <Link href="/overview" className="mb-6 flex h-8 w-8 items-center justify-center rounded-lg bg-blue-600 text-xs font-bold text-white">
          Y
        </Link>
            ...
            >
              ...
              <span
                className={`absolute -right-0.5 -top-0.5 h-2.5 w-2.5 rounded-full border-2 border-white dark:border-gray-900 ${
                  statusColors[s.status as ServerStatus] || "bg-gray-400"
                }`}
              />
            </button>
          ))}
  ...
  return (
    <aside className="flex h-full w-60 flex-col border-r bg-white dark:border-gray-800 dark:bg-gray-900">
      ...
      <div className="flex h-14 items-center gap-2 border-b px-4 dark:border-gray-800">
        <Link href="/overview" className="flex h-8 w-8 items-center justify-center rounded-lg bg-blue-600 text-xs font-bold text-white">
          Y
        </Link>
