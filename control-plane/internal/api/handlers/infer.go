...
func (inf *Infer) DeployInference(w http.ResponseWriter, r *http.Request) {
	...
	// enqueueing for offline delivery ÎÃÃ¶ enqueueing here too would create a
...
func (inf *Infer) RunBenchmark(w http.ResponseWriter, r *http.Request) {
	...
	if err := ws.QueueOrSendCommand(inf.Hub, inf.DB, serverID, cmd); err != nil {
		slog.Warn("queue/send run_benchmark", "error", err, "command_id", cmdID)
	}
...
// GetBenchmarks is an alias for GetBenchmarkResults (legacy /benchmarks path).
func (inf *Infer) GetBenchmarks(w http.ResponseWriter, r *http.Request) {
	inf.GetBenchmarkResults(w, r)
}
...
// ((generic - optimized) / generic) âÃ¹ 100 ÎÃ¥Ã positive = faster.
...
func (inf *Infer) GetInferenceStatus(w http.ResponseWriter, r *http.Request) {
	...
	// as a string and parse ÎÃÃ¶ the SQLite driver can't scan TEXT into time.Time.
