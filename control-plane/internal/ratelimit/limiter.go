...
// Default limits from the Layer 5A Step 8A plan: login is capped both per IP
// (10/15min) and per email (5/15min); forgot-password and reset-password are
// also rate limited to blunt brute force and email-bomb abuse. No account is
// ever locked — a legit user is only asked to wait a few minutes.
const (
	LoginIPLimit     = 10
	LoginEmailLimit  = 5
	ForgotIPLimit    = 5
...
func New() *Limiter {
	return NewWithLimits(LoginIPLimit, LoginEmailLimit, ForgotIPLimit, ForgotEmailLimit, ResetIPLimit, Window)
}

// NewWithLimits builds a Limiter with explicit limits (tests use small
// numbers so they do not have to wait out the real window).
func NewWithLimits(loginIP, loginEmail, forgotIP, forgotEmail, resetIP int, window time.Duration) *Limiter {
	return &Limiter{
